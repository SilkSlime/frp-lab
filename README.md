...
2. [Теоретическая минутка](#2)
    ...
    - [Схема сети и взаимодействия](#5)
3. [Практика](#6)
    - [Подготовка окружения](#7)
    - [Клонирование репозитория и структура проекта](#8)
    - [Генерация сертификатов](#9)
    - [Запуск сервера (frps)](#10)
    - [Запуск клиента (frpc)](#11)
    - [Проверка соединения и траблшутинг](#12)
    - [Остановка и очистка стенда](#13)
4. [Дальнейшие шаги](#14)
    - [Советы по продвинутому использованию](#15)
    - [Гайд по развертыванию на реальных серверах](#16)
...
flowchart TB
    subgraph "Интернет"
        direction LR
        browser["Пользователь<br>(браузер / curl)"] -- TCP-запрос на порт 9003 --> frps(frps<br>Публичный IP)
    end

    subgraph "Приватная сеть (за NAT)"
        direction LR
        frpc(frpc) -- "Проксирует трафик" --> backend(Локальный сервис<br>echo-backend:8080)
    end

    frps -- "Перенаправляет трафик<br>в зашифрованный туннель" --> frpc
```
...
```mermaid
sequenceDiagram
    participant C as frpc (Клиент)
    participant S as frps (Сервер)

    Note over C,S: Оба участника доверяют одному и тому же<br>Корневому Центру Сертификации (CA)

    C->>S: 1. TCP-соединение на порт 7000
    S-->>C: 2. ServerHello + свой сертификат (server.crt)
    Note right of C: Клиент проверяет:<br>1. Сертификат сервера подписан доверенным CA.<br>2. Имя в сертификате (SAN) совпадает с 'server'.
    C-->>S: 3. ClientHello + свой сертификат (client.crt)
    Note left of S: Сервер проверяет:<br>1. Сертификат клиента подписан доверенным CA.
    S-->>C: 4. TLS Handshake OK
    C->>S: 5. FRP Login (внутри зашифрованного канала)
    S-->>C: 6. Туннель готов к работе!
```


## Теоретическая минутка

### 2.1. Как работает обратный прокси-туннель (FRP)?
...
### 2.2. Сертификаты, CA и магия mTLS
